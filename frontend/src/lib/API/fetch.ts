...
import type {EmitData, EmitMethod, EmitResult} from "./types";



export async function emitServer<T>(url: string, method: EmitMethod, data?: EmitData): Promise<EmitResult<T> | undefined> {
    ...
    try {
        ...
        const responseData: EmitResult<T> = await res.json() as EmitResult<T>;
        return responseData;
