import type { IConfigurationLength, IValidationFunctions } from '../types/services/IValidator';

export function validateLength(value: string, rules: IConfigurationLength): boolean {
...
export const validatorFunctions: IValidationFunctions = {
  trim: validateTrim,
...
export function getPersonAgeByDate(date: string): number | null {
  ...
  const yearlyAge: number = today.getFullYear() - birthYear;
