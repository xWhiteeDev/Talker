  ...
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const requirement in requirements) {
        ...
        if (
          requiredElement.type === 'object' &&
          (Object.keys(passedElement).length > requiredElement.maxLength ||
            Object.keys(passedElement).length < requiredElement.minLength)
        ) {
          throw new Error(`Property: ${requirement} do not satisfy length requirements!`);
        }
        if (
          requiredElement.type === 'array' &&
          (passedElement.length > requiredElement.maxLength || passedElement.length < requiredElement.minLength)
        ) {
          throw new Error(`Property: ${requirement} do not satisfy length requirements!`);
        }
